...
Headless Chrome with `--disable-gpu` exposed a cost the GPU had been hiding: the night skin's darkness pass blurred every lit sign live under a `lighter` composite, and in software that blur is paid over the whole frame for each sign, each frame. A 300 ms virtual-time run took over 400 s of wall time; the day skins took under a second. The reveals are now baked once per sign size into cached sprites (`revealSprite` in `engine.js`), and a 3000 ms run of the night skin takes about 1 s, in line with the other skins. The measurement method is the two `tools/` scripts and `brand/backstage/probes/__slime3d-cost.html`: time a headless run at two frame counts and divide the difference, since virtual time freezes the page's own clocks.
