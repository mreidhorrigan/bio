...
(function () {
  ...
  function dropdown(label, items) {
    ...
    dd.className = "mh-dd";
    dd.dataset.menu = label;          // the English name: ?menu= and i18n.js match on THIS,
                                      // not on the visible label, which may be translated
  ...
  /* The English/French switch. Pages that ship <script src="i18n.js"> get it a beat
     sooner (less to redraw); this is the safety net for the ones that do not, which
     matters most for Horrigan_CV.html, whose markup is generated by the CV builder
     and would lose a hand-added script tag on the next build. */
  function injectI18n() {
    if (window.MH_I18N || document.querySelector('script[src*="i18n.js"]')) return;
    ["i18n.js", "i18n-fr.js"].forEach(function (file) {
      var s = document.createElement("script");
      s.src = new URL(file, HERE).href;
      s.async = false;                  // i18n.js must run before its dictionary
  ...
  function mount() {
    ...
    try {
      ...
      if (wantMenu) Array.prototype.forEach.call(bar.querySelectorAll(".mh-dd"), function (dd) {
        var name = dd.dataset.menu || (dd.querySelector("summary") || {}).textContent || "";
        if (name.trim().toLowerCase() === wantMenu.trim().toLowerCase()) { dd.setAttribute("open", ""); fitMenu(dd); }
      });
    ...
    document.dispatchEvent(new CustomEvent("mh:menubar", { detail: { bar: bar, nav: nav } }));
  }
